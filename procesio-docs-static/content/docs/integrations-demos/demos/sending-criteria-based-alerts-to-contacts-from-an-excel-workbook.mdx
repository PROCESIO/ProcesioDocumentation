...
- a configuration to an email server that will be used to send the email. ([see here](/docs/overview/processes/untitled/working-with-credentials))
- a configuration to a REST API of an web-to-sms service. ([see here](/docs/overview/processes/untitled/working-with-credentials/rest-api-credential)) 
...
**Step 2:&#xA0;**&#x43;onfigure the **Get File Data** according to [documentation](/docs/platform-actions/call-api/get-file-data). For configuring this step [video available here](https://www.youtube.com/watch?v=aLMgtMHQjb4).

**Step 3:&#xA0;**[Configure](/docs/platform-actions/call-api/for-each) the **For Each**
...
**Step 4:&#xA0;**&#x43;onfigure the "Decisional" action within the "For Each" border. Each case of the [Decisional](/docs/platform-actions/call-api/decisional) needs to be configured.
...
**Case 1**: we will use <span style={{color:'#2166ae'}}>&lt;%CustomerCsvLine.price%&gt;</span>, defined in the "For Each" output (mentioned in Step 3).

- We will set the <span style={{color:'#2166ae'}}>&lt;%customercsvline.price%&gt;</span> to be greater than or equal to 100.
...
**Case 2**: we will use <span style={{color:'#2166ae'}}>&lt;%CustomerCsvLine.price%&gt;</span>, defined in the "For Each" output (mentioned in Step 3).

- We will set the <span style={{color:'#2166ae'}}>&lt;%customercsvline.price%&gt;</span> to be less than 100.
...
- As mentioned at the beginning of this article, connections for "Call Api Action" as well as "Send Email" have to be created within "Credentials Manager". See in this [article](/docs/overview/processes/untitled/working-with-credentials) how to set up connections in Credentials Manager for  "Call Api Action" as well as "Send Email".
...
- In accordance with [SMSO API documentation](https://api-docs.smso.ro/#introduction), we will use the following Query Params:
  - To: <span style={{color:'#2166ae'}}>&lt;%CustomerCsvLine.phonenumber%&gt;</span> (leveraging the phone number contained in the excel file)
...
- **To:** <span style={{color:'#2166ae'}}>&lt;%CustomerCsvLine.email%&gt;</span> (leveraging the e-mail address contained in the excel file).
