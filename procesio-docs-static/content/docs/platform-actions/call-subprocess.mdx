...
To configure &#x61;**&#x20;Send email** action you may check [Send email action](/docs/platform-actions/call-api/send-email) article.
...
To configure the **Get File Data** action you may check the [Get File Data](/docs/platform-actions/call-api/get-file-data) article. 

Two process variables were created:

![](/images/docs/KwR-Rl75pVdWAMxDQFtNj_screenshot-2022-06-02-at-170048.png)

- **<span style={{color:'#1679e1'}}>getProducts</span>&#x20;**(type **list** of `orderedProducts`); data model that matches the structure of the excel file that will capture the data from the file at runtime.
...
Next, in order to ensure that a separate email is sent to every entry in the file, a ForEach action is needed. Check [For Each](/docs/platform-actions/call-api/for-each) article for configuration details.

1. Select the list to iterate through, i.e. <span style={{color:'#1679e1'}}>getProducts</span>``
2. Set the <span style={{color:'#2166ae'}}>iterator</span> name (you can name it <span style={{color:'#2166ae'}}>iterator</span> etc)
...
<Callout type="info">
As our action is inside a foreach border, use the <span style={{color:'#2166ae'}}>iterator</span>'s attributes when mapping the data, not the attributes of the data model that captures the data from the file.
</Callout>
