---
title: "List Variance"
icon: "⭐"
description: "This action computes the variance, which measures the amount of variation or dispersation in a sample of data points."
---

This action computes the variance, which measures the amount of variation or dispersation in a sample of data points.

![](/images/docs/H3yM3Nbz62nrf9o85LRJD_image.png)

## Inputs & Outputs

- **\[ I ] List of numbers**: <span style={{color:'#368b10'}}>Double (List)</span> representing data points for computing the variance.
- **\[ O ] Variance**: <span style={{color:'#368b10'}}>Double (Single Value)</span> representing the statistical measure of variability in the data set obtained from the input list.

![](/images/docs/ovvXYwhrMkRVJaMzU-aBM_image.png)

<Callout type="info">
The List Variance action uses the Sample Variance formula:

![](/images/docs/943VKSLqgQDUEVylgYuaJ_image.png)
</Callout>

## Example Usage

- Drag the List Variance action from the Community-Made folder to the canvas.

![](/images/docs/vdGUODHmzqoQAlbWJEAFm_image.png)

- Declare the variables to be used with the List Variance action.

![](/images/docs/X9fqa3sVaOzCfHssOzutr_image.png)

- Assign the newly created variables.
- Save & Run the process and then Check Instance to see the output.

![](/images/docs/JRSnwEMxFHrB5z2-e8D8U_image.png)

## Use Cases

- In market research, you may use the List Variance action to analyze survey data related to consumer preferences or behavior. By understanding the variation in responses, you can tailor your marketing strategies and product offerings to specific target audiences, making informed decisions to improve your business performance.

## Tips

- If you have a sample of data from a larger population, you should use the [List Variance](/docs/platform-actions/call-api/list-variance) action to correct for the bias introduced by using a smaller sample, this action is best used when dealing with a limited set of data. If you are dealing with a full population (i.e., you have data for every member of the population), you should NOT use the List Variance action.&#x20;

## Creator

- **Franck -&#x20;**[The Productivity Lab](https://theproductivitylab.io/)
